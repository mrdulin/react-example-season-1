...
class ComponentDidUpdateInputFocus extends Component {
  ...
  render() {
    ...
    return <div>
      ...
      <hr/>
      <input ref={ref => this._input = ref} type="text" value={this.state.value} onFocus={this.handleOnFocus} onChange={this.handleChange}
             onBlur={this.handleOnBlur} onClick={this.handleOnClick}/>
      <button style={{display: this.state.isInputing ? 'inline-block' : 'none'}} type='button' onMouseDown={this.handleDelete}>mousedown delete
      </button>
      ...
      <p>使用<code>onMouseDown</code>可以实现，使用<code>onClick</code>可以通过<code>componentDidUpdate</code>，再次让<code>input</code>聚焦，这时候会触发<code>onClick</code>事件。
      </p>
