  ...
  render() {
    ...
    return <div>
      <div>
        ...
        <p>报错：<code>Warning: Failed form propType: You provided a `value` prop to a form field without an `onChange` handler. This will render a
          read-only field. If the field should be mutable use `defaultValue`. Otherwise, set either `onChange` or `readOnly`. Check the render method
          of `InputTypeText`.</code></p>
        <p>设置了<code>input</code>的<code>value</code>属性，则这是一个controlled component，用户将不能与之交互，不能改变它的值。</p>
        <div>
          数据：
          <pre>this.state.value: {value}</pre>
        </div>
      </div>
      <div>
        <h3>例2：</h3>
      ...
      <div>
        ...
        <div>
          数据：
          <pre>this.state.textNull: {JSON.stringify(textNull)}</pre>
        </div>
      </div>
      <div>
        <h3>例4：</h3>
        ...
        <p>本来你想要一个controlled component，即设置了<code>value</code>属性值的input，用户不能改变其值。但是意外地，<code>value</code>属性被设置为<code>undefined</code>，这时候用户依旧可以改变其值。
        </p>
        <div>
          数据：
          <pre>this.state.textUndefined: {typeof textUndefined === 'undefined' ? 'undefined' : textUndefined}</pre>
        </div>
      </div>
      <div>
        <h3>例5：</h3>
        ...
        <div>
          数据：
          <pre>this.state.textEmptyString: {textEmptyString}</pre>
        </div>
      </div>
      <div>
        <h3>例6：</h3>
