  ...
  render() {
    ...
    return (
      <div>
        ...
        <ul>
          {
            books.map((book, index) => <li key={index}>{book} <span style={{ cursor: 'pointer' }} onClick={e => this.handleDelete(e, index)}>-</span>
            </li>)
          }
        </ul>
        <p>当前页：{page}</p>
        ...
        <div>this.props.books：
        <pre><code>{JSON.stringify(books, null, 4)}</code></pre>
        </div>
        <hr />
        <p>验证结果：</p>
